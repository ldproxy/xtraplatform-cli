    ...
    public static Result generate(
        ...
        try {
            ...
            if (typeObject.getOrDefault("tileProvider", true)) {
                ...
                AutoEntityFactory autoFactory3 = getAutoFactory(ldproxyCfg, EntityType.PROVIDERS.toString(), tileProvider.getEntitySubType());

                Map<String, List<String>> types2 = types.containsKey("") ? types : Map.of("", new ArrayList<>(types.keySet()));

                TileProviderFeaturesData entityData3 = autoFactory3.generate(tileProvider, types2, ignore -> {
                });

                ldproxyCfg.writeEntity(entityData3);

                newFiles.add(ldproxyCfg.getDataDirectory().relativize(ldproxyCfg.getEntityPath(entityData3).normalize()).toString());
            }
